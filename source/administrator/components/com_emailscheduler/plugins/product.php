...
class EmailschedulerPluginProduct extends EmailschedulerPluginAbstract
{
    public function getTriggers($like = array())
    {
        ...
        $query->select($db->quoteName(array('condition', 'actions', 'access', 'params')))
            ->from($db->quoteName('#__emailscheduler_triggers'))
    ...
    public function doActions($actions, $params)
    {
        ...
        if(!empty($params['bcc'])) $recipients['bcc'] = $params['bcc'];
